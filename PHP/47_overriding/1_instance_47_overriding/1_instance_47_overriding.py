...
def run_scala_query(project_folder):
    ROOT_DIR = os.path.abspath('/TP_Automatic_Framework') #TODO: to fix. On docker is ok, but not for Linux
    my_dir = Path(ROOT_DIR) / Path('testability_patterns/PHP/47_overriding')
    cpg_php = os.path.abspath(Path(ROOT_DIR) / Path('joern/querydb-php'))
    scala_query = my_dir / Path('./47_methods.sc')

    # 1) Generate binary for each file of the project
    # 2) Scala query launch
    binary_folder = os.path.abspath(project_folder / Path('binary_for_cpg_overr'))
        ...
        for line in data.split("\n"):
            ...
            for m in methods:
                ...
                if cls in cs[1]:
                    ls_cls.append(cs[0])
            for f in ls_par:
                if f in ls_cls and f not in dt:
                    # res = res + 1
                    # print(dir)
                    if f[0] == "_" and f[1] == "_":
                        continue
                    try:
                        with open(dir, 'r', encoding='utf8') as fl3:
                            conts = fl3.read()
                        conts = conts.lower()
                        if "function " + f in conts:
